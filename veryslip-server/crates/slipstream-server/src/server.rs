...
pub async fn run_server(config: &ServerConfig) -> Result<i32, ServerError> {
    ...
    let compression = if config.compression_enabled {
        tracing::info!(
            "Compression enabled with level {}",
    ...
    // Initialize batch processor (always enabled for compatibility)
    let batch_processor = Some(Arc::new(BatchProcessor::new()));
    tracing::info!("Batch processing enabled (max {} packets per batch)", crate::batch::MAX_BATCH_SIZE);

    // Initialize metrics collector and start metrics server if enabled
    ...
    loop {
                ...
                match recv {
                    Ok((size, peer)) => {
                        ...
                        if let Some(ref m) = metrics {
                            m.record_bytes_received(size as u64);
                        }
        ...
        for slot in slots.iter_mut() {
            ...
            let final_payload = if let (Some(engine), Some(payload_data)) = (&compression, payload) {
                match engine.compress(payload_data) {
                    Ok((compressed, was_compressed)) => {
                        if was_compressed {
                            tracing::trace!("Compressed response: {} -> {} bytes", payload_data.len(), compressed.len());
                        }
            ...
            // Record metrics for bytes sent and query processed
            if let Some(ref m) = metrics {
                m.record_bytes_sent(response.len() as u64);
                
                // Extract domain from question name
                let domain = slot.question.name.trim_end_matches('.');
                let is_batched = batch_processor
                    .as_ref()
                    .map(|p| p.is_batched(final_payload.unwrap_or(&[])))
                    .unwrap_or(false);
                m.record_query(domain, is_batched);
            }
